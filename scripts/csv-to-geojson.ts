  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(inputPath)
      ...
      .on('data', (row: any) => {
        ...
        if (!Object.keys(normalizedRow).some(key => /latitude/i.test(key)) ||
  ...
  return {
    ...
    features: rows.map(row => ({
      ...
      properties: Object.fromEntries(
        Object.entries(row).filter(([key]) => !['Latitude', 'Longitude', 'UID'].includes(key)).map(([key, value]) => [cleanFieldName(key), value])
      )
    }))
  };
}
